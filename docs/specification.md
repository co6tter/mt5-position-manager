...
全数値入力欄には`-` / `+`を配置し、Minutesは1分、Amountは1.00、Percentは0.1ポイント、pips欄は1 pip単位で増減する。EntryのPrice欄・PriceモードのSL/TPの増減はSL/TPタブと同じ規約で、通常1 pip相当、Tick Sizeが大きい場合は最低1 tick単位とする。下限・上限を超える操作は境界値で止める。

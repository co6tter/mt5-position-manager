...
SL/TPのラベルから`pts`表記を廃し、`SL`・`TP`とする。両者はそれぞれ独立したPrice／Pointsモード切替を持ち、初期モードはPoints、値は0(未設定)として、既存のPoints入力の挙動を保つ。SLとTPは互いに独立に設定・キャンセルでき、キャンセルした側は価格を0にして送信し、対応するラインとラベルを非表示にする。キャンセルした側は、ユーザーが明示的に再設定するまでTimerや他方の値の編集で復活しない。

TPはAuto／Manual／Offの3状態を持つ。有効なSLが設定された時点で初めてAutoへ入り、SL未設定の間はTPを自動生成しない。Auto状態では、損失距離D＝abs(想定建値E−SL)と設定RRの倍率rから、BuyはE＋D×r、SellはE−D×rをTick Sizeへ丸めた価格をTPとし、SL・E・設定RRのいずれかが変わるたびに再計算する(丸め後にあらためて方向・価格制約を検証する)。ユーザーがTPを直接入力またはドラッグで確定するとManualへ切り替わり、以後SLや現在価格を動かしてもTP価格は据え置かれ、現在RRを再計算する。Pointsで直接入力したTPも確定時の絶対価格として保持し、有効価格のプレビューで確認できる。SLなしでも直接TPを設定でき、その場合は最初からManualとして扱う。TPをキャンセルするとOffになりTP=0として送信され、SLの再設定・Timer・タブ切替をまたいでもOffを維持し、ユーザーが新たにTPを設定するか、明示的な「自動復帰」操作を行うまで変化しない。SLをキャンセルした際にAutoで生成済みのTPが存在していれば、その時点の価格を保持したままManualへ切り替える(TPが消えることも、SLなしでAutoに留まり続けることもない)。RRの数値だけを変更してもManual・Offは解除されず、有効なSLがある場合に限り、明示的な自動復帰操作でAutoへ戻して設定倍率からTPを再生成する。SLがない状態で自動復帰を試みた場合は理由を表示し、SLを代わりに生成することはない。

設定RRは`1 : x`で表し、xは正の有限小数、初期値は1.0。設定RR(ユーザーが指定した倍率)と、実際に有効なSL/TP価格から都度算出する現在RRを区別する。現在RRは、Tick Size丸め後の価格から`1 : abs(TP−想定建値E)/abs(E−SL)`で求め、SLが損失側かつTPが利益側にあるときだけ有効な数値を表示する。SL未設定、TP未設定、価格差がゼロ、方向不正(SL/TPが想定建値に対して逆側にある)のいずれかに該当する場合は、理由とともに`N/A`または`Invalid`を表示し、有限の数値を偽って表示しない。ラインをドラッグしている間の候補価格に対しても、確定前の現在RRを同様に表示する。最終行では、その時点のLotとSL/TP価格から見積もったSL到達時の概算損失・TP到達時の概算利益を、口座通貨で注文ボタンと並べて表示する。この概算額はSwap・Commissionを含めない参考値で、SLまたはTPが未設定・無効なときは対応する額を`N/A`とする。
...
確定済みのEntryの下書き(注文種別、方向、各価格、数量方式、設定RR、SL/TPのキャンセル状態)は、タブ切替・折り畳み・リサイズをまたいで保持する。進行中のライン・ドラッグは、これらに加えて銘柄・注文種別・方向・数量方式の変更でも破棄する。EA再初期化時はEntryを初期状態(注文種別Market、方向Buy、数量方式Manual Lot、有効なSLがある場合のみTPをAuto・ない場合は未設定、SL/TPのキャンセルなし)へ戻す。

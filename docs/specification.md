...
Entryタブは上から「注文種別・方向」「Bid/Ask・注文価格」「数量方式・金額／割合・Lot」「SL・設定／キャンセル」「TP・設定／キャンセル」「設定RR・現在RR・自動復帰」「有効なSL/TP価格とTP状態」「概算損益・注文ボタン」の順に1行ずつ配置する。注文種別はMarket／Limit／Stopの3種類、方向はBuy／Sellの2方向で、組み合わせは6通りある。種別・方向の選択自体は発注操作を兼ねず、選択を変えてもその場では送信されない。組み合わせごとに専用の注文ボタンを用意し、送信はそのボタンを押した場合に限る。方向を切り替えると進行中のライン・ドラッグは中断し、既存のSL/TP価格や注文価格を自動で反転・補正することはない。

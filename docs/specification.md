...
ラベルにはDigitsを保持した価格、選択Ticketの`OrderCalcProfit()`による口座通貨の概算合計（Swap・Commission除外）、建値からのBuy/Sell別Volume加重平均points、件数を表示する。金額は口座通貨の表示桁数を使用し、1件でも計算失敗なら`N/A`とするがpointsは表示する。選択全件のBroker制約を満たさない候補は`Invalid`と表示する。選択なし、別銘柄、Pipsモード、タブ非表示・折り畳みでは該当ラインとラベルを隠す。不正入力や価格取得失敗はヒントへ理由を表示する。

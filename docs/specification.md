...
TPはAuto／Manual／Offの3状態を持つ。有効なSLが設定された時点で初めてAutoへ入り、SL未設定の間はTPを自動生成しない。Auto状態では、損失距離D＝abs(想定建値E−SL)と設定RRの倍率rから、BuyはE＋D×r、SellはE−D×rをTick Sizeへ丸めた価格をTPとし、SL・E・設定RRのいずれかが変わるたびに再計算する(丸め後にあらためて方向・価格制約を検証する)。ユーザーがTPを直接入力またはドラッグで確定するとManualへ切り替わり、以後SLを動かしてもTP価格は据え置かれ、変わるのは現在RRの表示だけになる。SLなしでも直接TPを設定でき、その場合は最初からManualとして扱う。TPをキャンセルするとOffになりTP=0として送信され、SLの再設定・Timer・タブ切替をまたいでもOffを維持し、ユーザーが新たにTPを設定するか、明示的な「自動復帰」操作を行うまで変化しない。SLをキャンセルした際にAutoで生成済みのTPが存在していれば、その時点の価格を保持したままManualへ切り替える(TPが消えることも、SLなしでAutoに留まり続けることもない)。RRの数値だけを変更してもManual・Offは解除されず、有効なSLがある場合に限り、明示的な自動復帰操作でAutoへ戻して設定倍率からTPを再生成する。SLがない状態で自動復帰を試みた場合は理由を表示し、SLを代わりに生成することはない。

...
3. **基準ボタンと行配置を追加する。** 既存の`CreateButton()`、`CreateNumericInput()`、オブジェクト登録を利用し、`HandleChartEvent()`、表示更新、`ApplyTabVisibility()`まで接続する。BE/TrailingのON/OFFの赤・緑は維持し、基準ボタンは通常の選択ボタンとして文字で現在値を示す。
   ...
   | 60 | `Break Even`、ON/OFF、Triggerの`- [値] +`、Lockの`- [値] +` |
   | 88 | `Trailing`、ON/OFF、Triggerの`- [値] +`、Distanceの`- [値] +` |
   | 120 | pips単位・Trigger=0時の説明 |

   - 基準ボタンは約150px幅、数値編集欄は約60px幅とし、既存の増減ボタン込みで配置する。機能名・ON/OFF・ラベルを含めた2組が560px内の1行に収まるよう間隔を決める。
   - 説明末尾と余白から`PM_PANEL_TRAIL_HEIGHT`を更新し、`ContentHeight()`と`UpdateStatusLayout()`によるStatus・パネル必要高の計算へ反映する。単にボタンを追加して本文高108pxを残さない。
   - 最大桁数と表示倍率を検証し、1行配置を維持したまま座標と入力幅を調整する。行構造の変更は新しいユーザー要件として明示された場合に限る。
...
- **UI部品の重なり・非表示漏れ** — Break EvenとTrailingの1行配置を自動チェックし、最大桁数・高DPI・全タブ切替を実機で確認する。

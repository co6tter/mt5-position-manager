...
20. 起動直後にSL/TPラインが出ず、`Set SL`だけでSLとRR 1:1のTP、`Set TP`だけでTPのみを作れることを確認する。SLなしの`Restore Auto`はTPを消さず、理由を表示する。
21. TPをPointsで直接入力して確定し、その後のTick更新・SL移動・RR変更でTPの絶対価格が変わらないことを確認する。TPドラッグ中は現在RRを更新し、確定までは入力欄を変更しない。
22. リスク方式のLotとMarketのPriceは読み取り専用となることを確認する。Risk／RR／価格欄を編集中にTimerを待ち、文字列を上書きせず、確定済みの値で表示を計算することを確認する。`2junk`などを残したまま注文ボタンを押した場合も送信せず、理由を表示する。
23. Limit／StopのPointsが注文価格基準で換算され、ライン・概算・リスクLot・実際の注文に同じ価格が使われることを確認する。Marketへ切り替えて戻ってもLimit／Stopの入力価格が残ることを確認する。

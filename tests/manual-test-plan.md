...
MT5を使わない補助確認は`python3 tests/run-price-editor-tests.py`で実行できる。実際のヘルパーをC++へ渡して共通テストを実行するもので、上記のMQL5コンパイル・Script実行や以下の実機ケースを代替しない。
...
4. Auto Close、Equity Guard、Trailの全数値欄で`-` / `+`を操作し、単位ごとの増減、下限・上限、手入力後の正規化を確認する。
5. Buy/Sellそれぞれで最新Bid/Ask基準のSL/TPプレビューを確認し、0 points、SLのみ、TPのみ、SL/TP両方をデモ口座で実行する。
6. 成功時にStatusとExpertsログへside、symbol、要求Lot、結果Lot、約定価格、Deal、Order、retcodeが記録されることを確認する。部分約定は失敗表示にならないことを確認する。
7. Lotへ空欄・文字列・0、SL/TP pointsへ小数・負数・文字列を入力し、注文要求が送信されず理由がStatusへ表示されることを確認する。Algo Trading無効、Stops Level違反でも同様に確認する。
8. SL/TPタブでPriceモードを選び、選択した同一銘柄のポジションにSL/TPライン、価格、概算金額、pointsが表示されることを確認する。ラインをドラッグしてValueへ反映されるが、`Set / Change`までは注文要求が出ないことを確認する。
9. 選択解除、別銘柄、Pipsモード、ポジション消失、Tick取得不可でラインが消え、古いドラッグ価格が入力欄へ戻らないことを確認する。Stops/Freeze Level違反の候補は`Invalid`と表示する。金額取得失敗の`N/A`と区別する。
10. 同一銘柄の複数Buy、複数Sell、Buy/Sell混在で、金額合計と方向別pointsが正しく表示されることを確認する。
11. タイトルバーの`-`で折り畳み、タイトルと`+`以外が消えることを確認する。展開ボタン以外のタイトルバーをドラッグして移動し、`+`で再展開できることを確認する。
12. チャートSymbolに複数の同方向ポジションを作り、Lot加重平均建値に幅1pxの薄いピンクのEquity / Break-even破線がチャート背景に表示され、操作パネルを覆わないことを確認する。
13. Buy/Sell混在時はネットポジションの損益分岐価格へ移動し、ネットLotが0または対象ポジションが0件になるとラインが消えることを確認する。
14. タブ切替、パネル折り畳み、移動、リサイズ、時間足変更を行ってもラインが表示され、ポジション変更後1秒以内に更新されることを確認する。
15. パネルを任意の位置へ移動してから時間足を変更し、パネルが左上へ戻らず同じ位置に留まることを確認する。チャートを再読み込みしても位置が維持されることを確認する。
...
6. SL/TPタブの最小幅560pxと拡大幅で、各行が入力欄、`Set / Change`、`Clear`の順に一行で収まり、説明・Statusと重ならないことを確認する。
...
8. Tick Sizeが1 pipより大きい銘柄で、Priceの`+` / `-`が最低1 tick進み、Tick Sizeに合わない手入力値からも方向が逆転しないことを確認する。手入力→増減→ドラッグ→`Set / Change`を往復し、ドラッグ中・終了時の取引要求が0件、適用時は最新状態で全選択Ticketを検証することをログで確認する。

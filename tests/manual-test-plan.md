...
3c. Limit／Stopサブタブを開いた直後、Price欄が空でSL/TPのモード切替ボタンが銀文字（無効）になり、ヒントに注文価格が必要な旨が出ることを確認する。Price欄の`+`を押すと現在価格がシードされて1段階動き、同時にモード切替ボタンが白文字（有効）へ戻ることを確認する。`9x5`のような文字列を入れて`+`を押しても値が書き換わらず理由が出ることを確認する。

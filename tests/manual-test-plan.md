...
5. SL/TPラインとラベルを保有ポジション0件でドラッグし、価格へ反映されることを確認する。SLだけを設定してもTPのラインが出ないこと、TPドラッグ後の固定と現在RR表示を確認する。各ラベルがラインの直上に1行（価格 | Est. | RR）で表示され、下書きが無効なら末尾に`Invalid`、理由はパネルのヒントに出ることを確認する。
6. SL／TPを個別にキャンセルし、該当ラインだけが消え、もう一方の価格が残ることを確認する。`Clear SL`／`Clear TP`で該当欄が0に戻り、そのラインだけが消えることを確認する。TPキャンセル後にSLを動かしてもTPが復活しないこと、`Set SL`／`Set TP`で明示的に再設定できることを確認する。
...
11. SL/TPタブの初期値が0でラインが出ないこと、SLにだけ値を入れるとSLラインだけが出てTPラインは出ないことを確認する。Priceモードで選択した同一銘柄のポジションに、価格、概算金額、pointsがラインの直上の1行ラベルで表示されることを確認する。`Clear SL`／`Clear TP`で欄が0に戻り、そのラインが消えることを確認する。ラインをドラッグしてValueへ反映されるが、`Set / Change`までは注文要求が出ないことを確認する。
...
20. 起動直後にSL/TPラインが出ず、`Set SL`だけでSLのみ、`Set TP`だけでTPのみを作れることを確認する。RR入力行とAuto TPボタンが表示されないことを確認する。
...
22. リスク方式のLotとMarketのPriceは読み取り専用となることを確認する。Risk／価格欄を編集中にTimerを待ち、文字列を上書きせず、確定済みの値で表示を計算することを確認する。`2junk`などを残したまま注文ボタンを押した場合も送信せず、理由を表示する。
23. Limit／StopのPipsが注文価格基準で換算され、ライン・概算・リスクLot・実際の注文に同じ価格が使われることを確認する。Marketへ切り替えて戻ってもLimit／Stopの入力価格が残ることを確認する。
24. EntryとSL/TPタブで別々の入力値を用意し、片方のラインをドラッグしても他方の値が変わらないことを確認する。タブ切替・折り畳み・リサイズ・注文種別・方向・数量方式の変更でドラッグが破棄され、チャートスクロールが元へ戻ることを確認する。
25. 560pxの最小パネル幅、100%・125%・150%表示倍率で、Entry全行の入力欄・ボタン・プレビュー・Statusが重ならず操作できることを確認する。同条件でTrailの各機能が1行を維持し、初期BasisがPer Position、4数値欄が0であることを確認する。
